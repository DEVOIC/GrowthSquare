import React from "react";

const Stats = () => {
  return (
    <div className="w-screen bg-lightblue relative overflow-hidden py-7">
      <div className=" space-y-2">
        <div className="bg-white bg-opacity-10 before-marquee">
          <div className="   animate-marquee">
            <p className="text-white text-xl font-semibold"> 500+ Learners</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">75+ Members</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              11+ Incollage Presence
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              10+ Offline events
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              8+ Startup partners
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              5+ Hackathon collaboration{" "}
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">5+ Courses </p>
            <p className="text-white text-xl font-semibold">.</p>
          </div>
          <div aria-hidden="true" className="   animate-marquee">
            <p className="text-white text-xl font-semibold"> 500+ Learners</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">75+ Members</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              11+ Incollage Presence
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              10+ Offline events
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              8+ Startup partners
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              5+ Hackathon collaboration{" "}
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">5+ Courses </p>
            <p className="text-white text-xl font-semibold">.</p>
          </div>
        </div>
        <div className="bg-white bg-opacity-10 before-marquee-reverse">
          <div className="   animate-marquee-reverse">
            <p className="text-white text-xl font-semibold"> 500+ Learners</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">75+ Members</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              11+ Incollage Presence
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              10+ Offline events
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              8+ Startup partners
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              5+ Hackathon collaboration{" "}
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">5+ Courses </p>
            <p className="text-white text-xl font-semibold">.</p>
          </div>  
          <div aria-hidden="true" className="   animate-marquee-reverse">
            <p className="text-white text-xl font-semibold"> 500+ Learners</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">75+ Members</p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              11+ Incollage Presence
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              10+ Offline events
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              8+ Startup partners
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">
              5+ Hackathon collaboration{" "}
            </p>
            <p className="text-white text-xl font-semibold">.</p>
            <p className="text-white text-xl font-semibold">5+ Courses </p>
            <p className="text-white text-xl font-semibold">.</p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Stats;
